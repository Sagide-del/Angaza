...
function formatFor(mime = '') {
  ...
  if (mime.includes('word')) return 'word'; // .doc (msword) / .docx (wordprocessingml)
...
// Creates a short-lived Supabase "signed upload URL" so the browser can PUT a
// file straight to Supabase, bypassing our own server entirely. This matters
// because Vercel serverless functions hard-cap request bodies at 4.5MB — fine
// for one file, but too small once several files are sent in one request (as
// with bulk upload). Only relevant when Supabase Storage is configured.
async function createSignedUpload(originalName) {
  if (!useSupabase) return null;
  const name = slug(originalName || 'file');
  const unique = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}-${name}`;
  const objectPath = `jua/${unique}`;
  const signUrl = `${SUPABASE_URL}/storage/v1/object/upload/sign/${SUPABASE_BUCKET}/${objectPath}`;

  const { data } = await axios.post(signUrl, {}, {
    headers: {
...
module.exports = { saveFile, formatFor, useBlob, useSupabase, createSignedUpload };
